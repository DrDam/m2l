...
// Test TWR
function testTwr(Thrust, Mass, twr_target, Go, atm) {
    var currentTwr = Thrust / Mass / Go;
    var alt = getAltFromAtm(atm);

    var percent = twr_target.step * Math.ceil(alt / 10000);
    var twr_min = twr_target.min * (1-percent);

    if (!twr_target.max) {
        return currentTwr > twr_min;
    }
    else {
        return (currentTwr > twr_min && currentTwr < twr_target.max);
    }
}
...
function getMinMassInSOI(engine, twr, SOI) {
    let curve = getCaractForAtm(engine.curve, 10000);
    return curve.Thrust / SOI.Go / (twr.max + (twr.spread / 100));
}
