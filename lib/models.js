...
function getTrajectoryState(Dv_from_ground, trajectory){
    ...
    for(let trajectoryStep in trajectory) {
        ...
        if(trajectory[trajectoryStep].dv <= Dv_from_ground && trajectory[trajectoryStep+1].dv > Dv_from_ground) {
            ...
            let twrDeriv = (trajectory[trajectoryStep+1].twrReduction - trajectory[trajectoryStep].twrReduction) / dvDiff;

            return  {
                twrReduction: twrDeriv * Dv_from_ground + trajectory[0].twrReduction,
...
function reduceTwr(twr, reduction) {
    output = (1 - (reduction / 100)) * twr;

   // console.log(twr, reduction, output);
