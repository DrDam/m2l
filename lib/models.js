 ...
 */
function getMaxMassInVacuum(engine, twr, SOI, trajectory){
    let out_trajectory = getTrajectoryState(100000000, trajectory);
    let curve = getCaractForAtm(engine.curve, 0);
    let corrected_twr = reduceTwr(twr.min, out_trajectory.twrReduction) * (1 - (twr.spread / 100));
    return round(curve.Thrust / SOI.Go / corrected_twr);
}



function getMinMassInSOI(engine, twr, SOI, trajectory) {
    let out_trajectory = getTrajectoryState(0, trajectory);
