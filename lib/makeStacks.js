...
function generateTanksStacks(tanks, adapters, maxParts) {
    ...
    let CleanSubOptimals = function(localStack, stacksList) {
        let keys = [
            localStack.info.mass.full,
            localStack.info.mass.empty,
            localStack.info.stackable.bottom,
            localStack.info.stackable.top
        ];
        let localKey = keys.join('--');

        for (let stacksListKey in stacksList) {

            let testStack = stacksList[stacksListKey];
            let testKeys = [
                testStack.info.mass.full,
                testStack.info.mass.empty,
                testStack.info.stackable.bottom,
                testStack.info.stackable.top
            ];
            let testKey = testKeys.join('--');
            ...
            if (localKey === testKey) {
                if (localStack.info.nb >= testStack.nb) {
                    ...
                    stacksList.splice(stacksListKey, 1);
                }
            }
        }
